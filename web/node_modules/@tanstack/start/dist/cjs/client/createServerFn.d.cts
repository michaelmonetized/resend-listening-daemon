import { AnyValidator, Constrain } from '@tanstack/react-router';
...
export type FetcherData<TResponse> = WrapRSCs<TResponse extends JsonResponse<infer TData> ? TData : TResponse>;
export type WrapRSCs<T> = T extends JSX.Element ? ReadableStream : T extends Record<string, any> ? {
...
export type ServerFn<TMethod, TMiddlewares, TValidator, TResponse> = (ctx: ServerFnCtx<TMethod, TMiddlewares, TValidator>) => Promise<TResponse> | TResponse;
...
type ServerFnBase<TMethod extends Method = 'GET', TResponse = unknown, TMiddlewares = unknown, TValidator = unknown> = {
    ...
    validator: <TValidator>(validator: Constrain<TValidator, AnyValidator>) => Pick<ServerFnBase<TMethod, TResponse, TMiddlewares, TValidator>, 'handler' | 'middleware'>;
...
export declare function createServerFn<TMethod extends Method = 'GET', TResponse = unknown, TMiddlewares = undefined, TValidator = undefined>(options?: {
    method: TMethod;
